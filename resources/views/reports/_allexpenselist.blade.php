    <table id="tableWithSearch" class="table table-hover table-bordered" cellspacing="0" width="100%">
    ...
    @foreach($lists as $list)
        <tr>
            <td>{{ $list->date }}</td>
            <td>{{ $list->banks->account }} ({{ substr($list->banks->acc_number, -4) }})</td>
            <td>{{ $list->type }}</td>
...
{{--            <td><a href="{{ route('deposit.edit', $list->id) }}">{{ $list->description }}</a> </td>--}}
        </tr>
    @endforeach
